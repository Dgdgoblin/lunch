...
# -*- coding: utf-8 -*-
"""
Genererar en statisk index.html med 'Dagens lunch by PMO IT' från FEI & Cirkeln.
Körs schemalagt (GitHub Actions/cron) eller manuellt. Ingen server krävs.
"""
import re
import time
...
USER_AGENT = "Mozilla/5.0 (compatible; PMO-IT-Lunch/1.0)"
...
def parse_cirkeln(soup: BeautifulSoup, weekday_upper: str) -> list[str]:
    ...
    wd_pattern = "|".join(wd_order)
    day_name_cap = weekday_upper.capitalize()

    m = re.search(
        rf"{day_name_cap}\s*(.+?)(?=(?:{wd_pattern})\s*|Kontakt|Öppettider|Veckans vegetariska|$)",
        full,
...
def get_menu_for(restaurant: str, url: str, weekday_upper: str) -> DayMenu:
    soup = fetch(url)
    if restaurant.lower() == "fei":
        items = parse_fei(soup, weekday_upper)
    elif restaurant.lower() == "cirkeln":
        items = parse_cirkeln(soup, weekday_upper)
    else:
        items = []
    return DayMenu(restaurant=restaurant, items=items)
...
<meta http-equiv="refresh" content="1800"> <!-- Auto-refresh var 30:e minut -->
...
"""
    for dm in day_menus:
        html += f"""    <div class="rest">
      <div class="name">{dm.restaurant}</div>
"""
        if dm.items:
        ...
        html += "    </div>\n"
    html += f"""    <div class="footer">Uppdaterad: {updated_at} &middot; Tidszon: Europe/Stockholm</div>
  </div>
</div>
"""
    return html
...
def main() -> int:
    ...
    for name, url in RESTAURANTS.items():
        try:
            day_menus.append(get_menu_for(name, url, weekday_upper))
        except Exception as e:
            day_menus.append(DayMenu(restaurant=name, items=[f"(fel vid hämtning: {e})"]))
